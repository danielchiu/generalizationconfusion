...
def get_loader(args, data_aug=True):
    ...
    if 'noisy_set' in args.val_sets and args.noise_rate > 0:
        val_sets.append(noisy_train_set_for_val)
    if 'test_set' in args.val_sets:
        val_sets.append(test_set)
    if 'noisy_train' in args.val_sets and args.noise_rate > 0:
    ...
    if 'noisy_val' in args.val_sets and args.noise_rate > 0:
    ...
    return train_loader, val_loaders, test_loader, train_set.num_classes, np.asarray(train_set.targets), np.asarray(clean_train_set.targets)
