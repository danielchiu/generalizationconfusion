...
def get_loss(args, device, labels=None, num_classes=10):
    if args.loss == 'ce':
        criterion = CrossEntropy()
    
    elif args.loss == 'sat':
        criterion = SelfAdaptiveTrainingCE(labels, device=device, num_classes=num_classes, momentum=args.sat_alpha, es=args.sat_es)
    
    elif args.loss == 'sat_sce':
        alpha, beta = 1, 0.3
        criterion = SelfAdaptiveTrainingSCE(labels, device=device, num_classes=num_classes, momentum=args.sat_alpha, es=args.sat_es, alpha=alpha, beta=beta)
